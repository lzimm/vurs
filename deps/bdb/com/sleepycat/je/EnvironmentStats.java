...
package bdb.com.sleepycat.je;

import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_BACKLOG;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_CLUSTER_LNS_PROCESSED;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_DELETIONS;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_ENTRIES_READ;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_FILE_DELETION_BACKLOG;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_INS_CLEANED;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_INS_DEAD;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_INS_MIGRATED;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_INS_OBSOLETE;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_LNQUEUE_HITS;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_LNS_CLEANED;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_LNS_DEAD;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_LNS_LOCKED;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_LNS_MARKED;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_LNS_MIGRATED;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_LNS_OBSOLETE;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_MARKED_LNS_PROCESSED;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_PENDING_LNS_LOCKED;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_PENDING_LNS_PROCESSED;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_REPEAT_ITERATOR_READS;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_RUNS;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_TOTAL_LOG_SIZE;
import static bdb.com.sleepycat.je.cleaner.CleanerStatDefinition.CLEANER_TO_BE_CLEANED_LNS_PROCESSED;
import static bdb.com.sleepycat.je.dbi.DbiStatDefinition.ENVIMPL_RELATCHES_REQUIRED;
import static bdb.com.sleepycat.je.dbi.DbiStatDefinition.MB_ADMIN_BYTES;
import static bdb.com.sleepycat.je.dbi.DbiStatDefinition.MB_DATA_BYTES;
import static bdb.com.sleepycat.je.dbi.DbiStatDefinition.MB_LOCK_BYTES;
import static bdb.com.sleepycat.je.dbi.DbiStatDefinition.MB_SHARED_CACHE_TOTAL_BYTES;
import static bdb.com.sleepycat.je.dbi.DbiStatDefinition.MB_TOTAL_BYTES;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.EVICTOR_BINS_STRIPPED;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.EVICTOR_EVICT_PASSES;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.EVICTOR_NODES_EVICTED;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.EVICTOR_NODES_SCANNED;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.EVICTOR_NODES_SELECTED;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.EVICTOR_REQUIRED_EVICT_BYTES;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.EVICTOR_ROOT_NODES_EVICTED;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.EVICTOR_SHARED_CACHE_ENVS;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.BIN_FETCH;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.UPPER_IN_FETCH;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.LN_FETCH;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.BIN_FETCH_MISS;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.UPPER_IN_FETCH_MISS;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.LN_FETCH_MISS;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.CACHED_UPPER_INS;
import static bdb.com.sleepycat.je.evictor.EvictorStatDefinition.CACHED_BINS;
import static bdb.com.sleepycat.je.incomp.INCompStatDefinition.INCOMP_CURSORS_BINS;
import static bdb.com.sleepycat.je.incomp.INCompStatDefinition.INCOMP_DBCLOSED_BINS;
import static bdb.com.sleepycat.je.incomp.INCompStatDefinition.INCOMP_NON_EMPTY_BINS;
import static bdb.com.sleepycat.je.incomp.INCompStatDefinition.INCOMP_PROCESSED_BINS;
import static bdb.com.sleepycat.je.incomp.INCompStatDefinition.INCOMP_QUEUE_SIZE;
import static bdb.com.sleepycat.je.incomp.INCompStatDefinition.INCOMP_SPLIT_BINS;
import static bdb.com.sleepycat.je.latch.LatchStatDefinition.LATCH_CONTENTION;
import static bdb.com.sleepycat.je.latch.LatchStatDefinition.LATCH_NOWAIT_SUCCESS;
import static bdb.com.sleepycat.je.latch.LatchStatDefinition.LATCH_NOWAIT_UNSUCCESS;
import static bdb.com.sleepycat.je.latch.LatchStatDefinition.LATCH_NO_WAITERS;
import static bdb.com.sleepycat.je.latch.LatchStatDefinition.LATCH_RELEASES;
import static bdb.com.sleepycat.je.latch.LatchStatDefinition.LATCH_SELF_OWNED;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_BYTES_READ_FROM_WRITEQUEUE;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_BYTES_WRITTEN_FROM_WRITEQUEUE;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_FILE_OPENS;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_OPEN_FILES;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_RANDOM_READS;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_RANDOM_READ_BYTES;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_RANDOM_WRITES;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_RANDOM_WRITE_BYTES;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_READS_FROM_WRITEQUEUE;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_SEQUENTIAL_READS;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_SEQUENTIAL_READ_BYTES;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_SEQUENTIAL_WRITES;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_SEQUENTIAL_WRITE_BYTES;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_WRITEQUEUE_OVERFLOW;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_WRITEQUEUE_OVERFLOW_FAILURES;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_WRITES_FROM_WRITEQUEUE;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FILEMGR_LOG_FSYNCS;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FSYNCMGR_FSYNCS;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FSYNCMGR_FSYNC_REQUESTS;
import static bdb.com.sleepycat.je.log.LogStatDefinition.FSYNCMGR_FSYNC_TIMEOUTS;
import static bdb.com.sleepycat.je.log.LogStatDefinition.LBFP_BUFFER_BYTES;
import static bdb.com.sleepycat.je.log.LogStatDefinition.LBFP_LOG_BUFFERS;
import static bdb.com.sleepycat.je.log.LogStatDefinition.LBFP_MISS;
import static bdb.com.sleepycat.je.log.LogStatDefinition.LBFP_NOT_RESIDENT;
import static bdb.com.sleepycat.je.log.LogStatDefinition.LOGMGR_END_OF_LOG;
import static bdb.com.sleepycat.je.log.LogStatDefinition.LOGMGR_REPEAT_FAULT_READS;
import static bdb.com.sleepycat.je.log.LogStatDefinition.LOGMGR_TEMP_BUFFER_WRITES;
import static bdb.com.sleepycat.je.recovery.CheckpointStatDefinition.CKPT_CHECKPOINTS;
import static bdb.com.sleepycat.je.recovery.CheckpointStatDefinition.CKPT_DELTA_IN_FLUSH;
import static bdb.com.sleepycat.je.recovery.CheckpointStatDefinition.CKPT_FULL_BIN_FLUSH;
import static bdb.com.sleepycat.je.recovery.CheckpointStatDefinition.CKPT_FULL_IN_FLUSH;
import static bdb.com.sleepycat.je.recovery.CheckpointStatDefinition.CKPT_LAST_CKPTID;
import static bdb.com.sleepycat.je.recovery.CheckpointStatDefinition.CKPT_LAST_CKPT_END;
import static bdb.com.sleepycat.je.recovery.CheckpointStatDefinition.CKPT_LAST_CKPT_START;
import static bdb.com.sleepycat.je.txn.LockStatDefinition.LOCK_OWNERS;
import static bdb.com.sleepycat.je.txn.LockStatDefinition.LOCK_READ_LOCKS;
import static bdb.com.sleepycat.je.txn.LockStatDefinition.LOCK_REQUESTS;
import static bdb.com.sleepycat.je.txn.LockStatDefinition.LOCK_TOTAL;
import static bdb.com.sleepycat.je.txn.LockStatDefinition.LOCK_WAITERS;
import static bdb.com.sleepycat.je.txn.LockStatDefinition.LOCK_WAITS;
import static bdb.com.sleepycat.je.txn.LockStatDefinition.LOCK_WRITE_LOCKS;

import java.io.Serializable;
import java.util.HashMap;
import java.util.Map;

import bdb.com.sleepycat.je.cleaner.CleanerStatDefinition;
import bdb.com.sleepycat.je.dbi.DbiStatDefinition;
import bdb.com.sleepycat.je.evictor.EvictorStatDefinition;
import bdb.com.sleepycat.je.evictor.Evictor.EvictionSource;
import bdb.com.sleepycat.je.incomp.INCompStatDefinition;
import bdb.com.sleepycat.je.log.LogStatDefinition;
import bdb.com.sleepycat.je.recovery.CheckpointStatDefinition;
import bdb.com.sleepycat.je.txn.LockStatDefinition;
import bdb.com.sleepycat.je.utilint.StatGroup;
...
public class EnvironmentStats implements Serializable {
     ...
     * bdb.com.sleepycat.je.util.DbBackup}.</li>
    ...
    /**
     * Number of BINs evicted from the cache by the background evictor
    ...
    /**
     * Number of BINs evicted from the cache by calls to {@link
     * Environment#evictMemory} thread. As a subset of nNodesExplicitlyEvicted,
    ...
    /**
     * Number of BINs evicted from the cache in due to an explicitly set {@link
    ...
    /**
     * Number of upper INs evicted from the cache by the background evictor
     * thread. As a subset of nNodesExplicitlyEvicted, it is an indicator of
     * what eviction is targeting and the activity that is instigating
     * eviction.
     */
    ...
    /**
     * Number of upper INs evicted from the cache by calls to {@link
     * Environment#evictMemory} thread. As a subset of nNodesExplicitlyEvicted,
     * it is an indicator of what eviction is targeting and the activity that
     * is instigating eviction.
     */
     ...
     * bdb.com.sleepycat.je.Environment#getStats Environment.getStats} is
     * called in "slow" mode.
     */
