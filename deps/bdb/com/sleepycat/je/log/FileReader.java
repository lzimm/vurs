...
public abstract class FileReader {
    ...
    protected static class ReadWindow {
        ...
        /* 
         * Assume that the window is properly positioned.  Fill the read buffer
         * with data from this file handle. Leave the read buffer pointing at
         * the target offset.
         */
        protected void fillFromFile(FileHandle fileHandle, long targetOffset) 
            ...
            readBuffer.clear();
            fileManager.readFromFile(fileHandle.getFile(), 
                                     readBuffer,
                                     startOffset,
            ...
            threadSafeBufferFlip(readBuffer);
            threadSafeBufferPosition(readBuffer, (int) (targetOffset - 
                                                        startOffset));

            /*
             * Ensure that fileNum and logVersion are in sync.  setFileNum
             * handles changes in the file number.  But we must also update the
             * logVersion here to handle the first read after we initialize
             * fileNum and logVersion is unknown.
             */
