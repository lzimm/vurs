...
import bdb.com.sleepycat.bind.EntityBinding;
import bdb.com.sleepycat.bind.tuple.StringBinding;
...
import bdb.com.sleepycat.je.ForeignKeyDeleteAction;
/* <!-- begin JE only --> */
import bdb.com.sleepycat.je.LockConflictException;
/* <!-- end JE only --> */
import bdb.com.sleepycat.je.OperationStatus;
import bdb.com.sleepycat.je.SecondaryConfig;
import bdb.com.sleepycat.je.SecondaryDatabase;
import bdb.com.sleepycat.je.Sequence;
import bdb.com.sleepycat.je.SequenceConfig;
/* <!-- begin JE only --> */
import bdb.com.sleepycat.je.SequenceExistsException;
import bdb.com.sleepycat.je.SequenceNotFoundException;
/* <!-- end JE only --> */
import bdb.com.sleepycat.je.Transaction;
import bdb.com.sleepycat.je.TransactionConfig;
import bdb.com.sleepycat.je.utilint.IdentityHashMap;
import bdb.com.sleepycat.persist.DatabaseNamer;
import bdb.com.sleepycat.persist.IndexNotAvailableException;
import bdb.com.sleepycat.persist.PrimaryIndex;
import bdb.com.sleepycat.persist.SecondaryIndex;
import bdb.com.sleepycat.persist.StoreConfig;
import bdb.com.sleepycat.persist.StoreExistsException;
import bdb.com.sleepycat.persist.StoreNotFoundException;
import bdb.com.sleepycat.persist.evolve.EvolveConfig;
import bdb.com.sleepycat.persist.evolve.EvolveEvent;
import bdb.com.sleepycat.persist.evolve.EvolveInternal;
import bdb.com.sleepycat.persist.evolve.EvolveListener;
import bdb.com.sleepycat.persist.evolve.EvolveStats;
import bdb.com.sleepycat.persist.evolve.IncompatibleClassException;
import bdb.com.sleepycat.persist.evolve.Mutations;
import bdb.com.sleepycat.persist.model.DeleteAction;
import bdb.com.sleepycat.persist.model.EntityMetadata;
import bdb.com.sleepycat.persist.model.EntityModel;
import bdb.com.sleepycat.persist.model.PrimaryKeyMetadata;
import bdb.com.sleepycat.persist.model.Relationship;
import bdb.com.sleepycat.persist.model.SecondaryKeyMetadata;
import bdb.com.sleepycat.persist.raw.RawObject;
...
public class Store {
    ...
    private static final String DB_NAME_PREFIX = "bdb.com.sleepycat.persist.";
    ...
    private static final TransactionConfig NO_WAIT_TXN_CONFIG;
    static {
        NO_WAIT_TXN_CONFIG = new TransactionConfig();
        NO_WAIT_TXN_CONFIG.setNoWait(true);
    }
    ...
    private Map<String, InternalPrimaryIndex> priIndexMap;
    private Map<String, InternalSecondaryIndex> secIndexMap;
                    ...
                    try {
                        ...
                        catalog = new PersistCatalog
                            (txn, env, storePrefix, storePrefix + CATALOG_DB,
                             dbConfig, model, config.getMutations(), rawAccess,
                             this);
                        ...
                        /*
                         * If the catalog is read-only we abort rather than
                         * commit, because a ReplicaWriteException may have
                         * occurred.  ReplicaWriteException invalidates the
                         * transaction, and there are no writes to commit
                         * anyway. [#16655]
                         */
                        if (txn != null) {
                            if (success && !catalog.isReadOnly()) {
                                txn.commit();
                            } else {
                                txn.abort();
                            }
    ...
    public synchronized <PK, E> PrimaryIndex<PK, E>
        ...
        if (priIndex == null) {
            ...
            if (dbConfig.getTransactional() &&
                DbCompat.getThreadTransaction(env) == null) {
                txn = env.beginTransaction(null, NO_WAIT_TXN_CONFIG);
            }
    ...
    public boolean attemptRefresh() {
        ...
        final PersistCatalog newCatalog =
            refresh(oldCatalog, -1 /*errorFormatId*/);
     ...
     * This is undesriable, but is not dangerous.  Sychrnoization must be
    ...
    synchronized PersistCatalog refresh(final PersistCatalog oldCatalog,
        ...
        boolean success = false;
        try {
            catalog = new PersistCatalog(txn, oldCatalog, storePrefix);
            success = true;
        } finally {
            if (txn != null) {
                if (success && !catalog.isReadOnly()) {
